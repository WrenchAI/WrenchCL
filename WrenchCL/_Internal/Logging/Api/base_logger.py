...
class BaseLogger:
    ...
    def info(
        ...
        *args: Any,
        ...
        self._log(
            level="INFO",
            args=args,
            no_format=opts.no_format,
            no_color=opts.no_color,
            stack_info=opts.stack_info,
            header=header,
        )
    ...
    def error(
        ...
        **kwargs: Any,
        ...
        self._log(
            level="ERROR",
            args=args,
            no_format=log_opts.no_format,
            no_color=log_opts.no_color,
            stack_info=log_opts.stack_info,
            header=header,
        )
    ...
    def reinitialize(self, verbose=False):
        ...
        if verbose:
            ...
            self._internal.log_internal(
                json.dumps(self.state, indent=2, default=lambda x: str(x), ensure_ascii=False)
            )
    ...
    def _log(
        self,
        level: Union[LogLevel, logLevels],
        args,
    ...
    def _pretty_log(self, obj: Any, compact: bool = False, **kwargs) -> None:
        ...
        try:
            if isinstance(obj, pd.DataFrame):
                ...
                output = (
                    json.dumps(obj, indent=indent, ensure_ascii=False, **kwargs)
                    if not compact
                    else pformat(obj, compact=True, width=cwidth)
                )
                ...
                raw = obj.json()
                output = (
                    json.dumps(raw, indent=indent, ensure_ascii=False, **kwargs)
                    if not compact
                    else pformat(raw, compact=compact, width=cwidth)
                )
            elif isinstance(obj, str) or hasattr(obj, "__repr__") or hasattr(obj, "__str__"):
                try:
                    parsed = json.loads(obj)
                    output = (
                        json.dumps(parsed, indent=indent, ensure_ascii=False, default=str, **kwargs)
                        if not compact
                        else pformat(parsed, compact=True, width=cwidth)
                    )
                except Exception:
                    output = obj
